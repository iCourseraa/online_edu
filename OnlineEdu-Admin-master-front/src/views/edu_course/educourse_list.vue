<template>
  <div class="app-container">
      ...
      <el-form ref="searchParamsForm" :inline="true" :model="searchParams" size="small">
        ...
        <el-form-item label="课程分类：" prop="subjectId">
          <el-cascader v-model="searchParams.subjectId" :options="subjectOptions"
            :props="{ expandTrigger: 'hover', value: 'id', label: 'title' }" :clearable="true" :filterable="true"
            placeholder="请选择课程分类" @change="subjectOptionsChange" />
        </el-form-item>
    ...
    <div>
      <el-table ref="listTable" v-loading="tableDataLoading" :data="listData" tooltip-effect="dark"
        ...
        <el-table-column prop="enable" label="上下架" width="180" align="center">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.enable" active-color="#13ce66" inactive-color="#ff4949" active-text="上架"
              inactive-text="下架" :active-value="true" :inactive-value="false"
        ...
        <el-table-column label="操作" width="160">
          <template slot-scope="scope">
            ...
            <el-popconfirm style="margin-left: 10px" placement="top-end" confirm-button-text="确定"
              cancel-button-text="取消" icon="el-icon-info" icon-color="red" :title="`你要永久删除课程[${scope.row.title}]吗？`"
    ...
    <div>
      <!-- 启用、禁用对话框 -->
      <el-dialog :title.sync="enableDialogTitle" :visible.sync="enableDialogVisible" destroy-on-close width="24%"
