    ...
    <div v-else>
      <el-row :gutter="20" class="course-list">
        <el-col :span="6" v-for="(course, index) in paddedFavoriteCourses" :key="index">
          <router-link v-if="!isVirtualCourse(course)" :to="{ path: '/course/detail/' + course.id }" class="course-item-link">
...
.course-image {
  width: 100%;
  height: 150px; /* 固定高度 */
  object-fit: contain; /* 等比例缩放并保持原始宽高比 */
  border: none; /* 移除边框 */
