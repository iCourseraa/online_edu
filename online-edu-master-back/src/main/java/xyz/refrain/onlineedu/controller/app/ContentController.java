...
public class ContentController {
	...
	@GetMapping("/get/course/isfavorite/{id}")
	@ApiOperation("判断学员是否已经订阅课程")
	public R getIsfavoriteCourse(@PathVariable("id") @Min(1) Integer id, HttpServletRequest request) {
		UctrMemberDetail member = SessionUtils.getMember(request);
		if (Objects.isNull(member)) {
			return new R(RS.NOT_LOGIN.status(), "请登录后再操作");
		}
//		RedisUtils.incr(StatConstant.COURSE_BUY_COUNT, 1);
		return eduCourseService.isfavoriteCourse(member.getId(), id);
	}
...
////////////////////////////////////////////////////////
		...
		String key_1 = StatConstant.COURSE_VIEW_COUNT + member.getId() + StatConstant.SEPARATOR + courseId;
		RedisUtils.incr(key, 1);
		RedisUtils.incr(key_1, 1);
////////////////////////////////////////////////////////////
		return r;
	}
