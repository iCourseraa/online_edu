...
import xyz.refrain.onlineedu.utils.*;
...
public class EduCourseService {
	...
	public R getDetails(int id) {
		...
		if(Objects.nonNull(id)){
			entity.setBuyCount(relCourseMemberMapper.selectTotal(id));
		}
	...
	public R pass(Integer id) {
		...
		int i = eduCourseMapper.update(null,
				Wrappers.lambdaUpdate(EduCourseEntity.class)
						...
						.set(EduCourseEntity::getUpdateStatus,"修改")
		);
		if (i > 0) {
...
//		);
//		if (count > 0){
//			return RUtils.fail("学员已收藏课程");
//		}
//		else{
//			RedisUtils.incr(StatConstant.COURSE_BUY_COUNT, 1);
//		}
//		RelCourseMemberEntity memberEntity = new RelCourseMemberEntity();
//		memberEntity.setCourseId(courseId);
//		memberEntity.setMemberId(memberId);
//		memberEntity.setCreateTime(LocalDateTime.now());
//		memberEntity.setIsFavorite(true);
//		relCourseMemberMapper.insert(memberEntity);
//		return RUtils.success("收藏成功");
//	}
	public R isBuyCourse(int memberId, int courseId) {
		// 查询是否已存在该学员和课程的记录
		RelCourseMemberEntity existingMember = relCourseMemberMapper.selectOne(
				Wrappers.lambdaQuery(RelCourseMemberEntity.class)
						.eq(RelCourseMemberEntity::getMemberId, memberId)
						.eq(RelCourseMemberEntity::getCourseId, courseId)
		);

		// 如果存在，检查 isFavorite 的状态
		if (existingMember != null) {
			if (existingMember.getIsFavorite()) {
				// 如果已收藏，返回失败
//				existingMember.setIsFavorite(true);
//				return RUtils.fail("学员已收藏课程");
				existingMember.setIsFavorite(false);
				existingMember.setUpdateTime(LocalDateTime.now());
				relCourseMemberMapper.updateById(existingMember);
				return RUtils.success("取消收藏成功");

			} else {
				// 如果未收藏，更新 isFavorite 为 true
				existingMember.setIsFavorite(true);
				existingMember.setUpdateTime(LocalDateTime.now());
				relCourseMemberMapper.updateById(existingMember);
				return RUtils.success("收藏成功");
			...
			// 如果没有记录，则插入新的记录
			RelCourseMemberEntity newMemberEntity = new RelCourseMemberEntity();
			newMemberEntity.setCourseId(courseId);
			newMemberEntity.setMemberId(memberId);
			newMemberEntity.setCreateTime(LocalDateTime.now());
			newMemberEntity.setIsFavorite(true);

			relCourseMemberMapper.insert(newMemberEntity);
			// 增加 Redis 购买计数
	...
	public R isfavoriteCourse(int memberId, int courseId) {
		// 查询是否已存在该学员和课程的记录
		RelCourseMemberEntity existingMember = relCourseMemberMapper.selectOne(
				Wrappers.lambdaQuery(RelCourseMemberEntity.class)
						.eq(RelCourseMemberEntity::getMemberId, memberId)
		...
		if (existingMember != null) {
			if (existingMember.getIsFavorite()) {
				return RUtils.success("已收藏");
			} else {
				return RUtils.success("未收藏");
			}
		} else {
			return RUtils.success("未收藏");
		}
		...
		List<EduCourseSimpleVO> voList = entityIPage.getRecords().stream()
				...
				.map(e -> {
					...
					if(Objects.nonNull(vo.getId())){
						vo.setBuyCount(relCourseMemberMapper.selectTotal(vo.getId()));
					}
					return vo;
				})
				.collect(Collectors.toList());
