...
CREATE TABLE `edu_course` (
  ...
  `price` double(10,2) unsigned DEFAULT '0.00' COMMENT '课程销售价格，设置为0则可免费观看',
  `lesson_num` int unsigned NOT NULL DEFAULT '0' COMMENT '总课时',
  `cover` varchar(1023) CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci NOT NULL DEFAULT '' COMMENT '课程封面图片路径',
  `description` text CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci COMMENT '课程描述',
  `buy_count` int unsigned DEFAULT '0' COMMENT '销售数量',
  `view_count` int unsigned DEFAULT '0' COMMENT '浏览数量',
  `sort` int DEFAULT '0' COMMENT '显示排序',
  ...
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_status` varchar(10) DEFAULT NULL COMMENT '修改狀態',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=28 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci ROW_FORMAT=COMPACT COMMENT='课程表';

-- 2. 删除并创建 `rel_course_member` 表
DROP TABLE IF EXISTS `rel_course_member`;
...
CREATE TABLE `rel_course_member` (
  ...
  UNIQUE KEY `idx_course_id` (`course_id`) USING BTREE,
  KEY `idx_member_course` (`member_id`,`course_id`) USING BTREE COMMENT '联合索引(member_id, course_id)'
) ENGINE=InnoDB AUTO_INCREMENT=26 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci ROW_FORMAT=DYNAMIC COMMENT='课程订阅-学员关系表';

-- 3. 删除并创建 `rel_course_study` 表
DROP TABLE IF EXISTS `rel_course_study`;

CREATE TABLE `rel_course_study` (
  `id` int unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
  `course_id` int unsigned NOT NULL DEFAULT '0' COMMENT '课程Id',
  `member_id` int unsigned NOT NULL DEFAULT '0' COMMENT '学员Id',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  ...
  PRIMARY KEY (`id`) USING BTREE,
  KEY `idx_member_course` (`member_id`,`course_id`) USING BTREE COMMENT '联合索引(member_id, course_id)'
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci ROW_FORMAT=DYNAMIC COMMENT='最近学习-学员关系表';

-- 4. 删除并创建 `tch_stat_daily` 表
DROP TABLE IF EXISTS `tch_stat_daily`;

CREATE TABLE `tch_stat_daily` (
  `id` int unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
  `date` datetime NOT NULL COMMENT '统计日期',
  `course_id` int unsigned NOT NULL DEFAULT '0' COMMENT '课程Id',
  `video_view_count` int unsigned DEFAULT '0' COMMENT '视频播放数',
  `course_buy_count` int unsigned DEFAULT '0' COMMENT '订阅数量',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE,
  KEY `statistics_day` (`date`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=1826 DEFAULT CHARSET=utf8mb3 ROW_FORMAT=DYNAMIC COMMENT='老师端统计日数据';

-- 5. 删除并创建 `edu_video_number` 表
DROP TABLE IF EXISTS `edu_video_number`;

CREATE TABLE `edu_video_number` (
  `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `user_id` VARCHAR(255) DEFAULT NULL,
  `video_id` VARCHAR(255) DEFAULT NULL,
  `play_number` VARCHAR(255) DEFAULT NULL,
  `pause_number` VARCHAR(255) DEFAULT NULL,
  `ended_number` VARCHAR(255) DEFAULT NULL,
  `seeked_number` VARCHAR(255) DEFAULT NULL,
  `completion_rate_number` VARCHAR(255) DEFAULT NULL,
  `curr_time` VARCHAR(255) DEFAULT NULL,
  `duration_number` VARCHAR(255) DEFAULT NULL,
  `kj_number` VARCHAR(255) DEFAULT NULL,
  `kt_number` VARCHAR(255) DEFAULT NULL,
  `create_time` DATETIME DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- 二次更新
-- 1. 删除现有索引（假设索引名为 idx_course_id，如果索引名不同，请替换成实际索引名）
DROP INDEX idx_course_id ON rel_course_member;

-- 2. 创建 NORMAL 索引（默认的 BTREE 索引）
CREATE INDEX idx_course_id ON rel_course_member (course_id);
